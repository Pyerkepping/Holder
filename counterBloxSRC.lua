...
local baseUrl = "https://auth15.vercel.app"

-- Check HWID
local function checkHWID()
    local response = game:HttpGet(baseUrl .. "/init-hwid?hwid=" .. gethwid())
    local success, result = pcall(function() 
        return HttpService:JSONDecode(response) 
    end)
    
    if not success or not result.success then
        game.Players.LocalPlayer:Kick(result.message)
        return false
    end
    return true
end
...
Mouse2.Button1Down:Connect(function()
    LocalPlayer.Additionals.TotalDamage.Changed:Wait()
    if Tracers.Options.Enabled and LocalPlayer.Additionals.TotalDamage.Value ~= 0 then
